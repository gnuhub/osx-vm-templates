...
DMG_BUILD_TIME=$(date +%Y%m%d_%H%m%S)
msg_status "OS X version detected: 10.$DMG_OS_VERS_MAJOR.$DMG_OS_VERS_MINOR, build $DMG_OS_BUILD"

OUTPUT_DMG="$OUT_DIR/OSX_InstallESD_${DMG_OS_VERS}_${DMG_OS_BUILD}_${DMG_BUILD_TIME}.dmg"
...
if [ $DMG_OS_VERS_MAJOR -ge 9 ]; then
	...
	hdiutil resize -size 8G "$BASE_SYSTEM_DMG_RW"
fi
